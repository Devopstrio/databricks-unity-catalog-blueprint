...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Anchor, Share2, Server, Repeat, AlertTriangle, Layers, BookOpen, GitGraph, FileText } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Unity IQ</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/catalogs" icon={<BookOpen size={20} />} label="Catalog Explorer" />
            <NavItem to="/access" icon={<ShieldCheck size={20} />} label="Access Governance" />
            <NavItem to="/lineage" icon={<GitGraph size={20} />} label="Lineage Center" />
            <NavItem to="/sharing" icon={<Share2 size={20} />} label="Sharing Portal" />
            <NavItem to="/risks" icon={<AlertTriangle size={20} />} label="Risk Center" />
            <NavItem to="/compliance" icon={<FileText size={20} />} label="Review Board" />
          </nav>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              <span>Databricks Blueprint</span>
              <span>/</span>
              <span className="text-white font-bold">Unified Governance Plane</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Governance Architect</p>
                <p className="text-[10px] text-blue-400 uppercase tracking-widest font-black">Industrialized UC</p>
              </div>
